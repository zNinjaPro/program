...
pub fn hash_two(left: &[u8; 32], right: &[u8; 32]) -> Result<[u8; 32]> {
    poseidon_hash2(*left, *right)
        .map_err(|_| ShieldedPoolError::PoseidonHashError.into())
}

/// Poseidon 2-arity hash using direct syscall on-chain, hashv off-chain.
/// Normalizes inputs to BN254 field before hashing.
fn poseidon_hash2(left: [u8; 32], right: [u8; 32]) -> Result<[u8; 32]> {
    ...
    #[cfg(not(target_os = "solana"))]
    {
        ...
        match hashv(Parameters::Bn254X5, Endianness::BigEndian, &inputs) {
            Ok(h) => Ok(h.to_bytes()),
            Err(_e) => Err(ProgramError::InvalidArgument.into()),
        }
    }
}
...
pub fn compute_zero_hashes(_depth: usize) -> Result<[[u8; 32]; MERKLE_DEPTH]> {
    #[cfg(target_os = "solana")]
    {
...
pub fn compute_zero_hashes_legacy(_depth: usize) -> Result<[[u8; 32]; LEGACY_MERKLE_DEPTH]> {
    #[cfg(target_os = "solana")]
    {
...
pub fn compute_root_epoch(tree: &EpochTree) -> Result<[u8; 32]> {
    let depth = tree.depth as usize;
    let mut idx = tree.next_index;
    
    if idx == 0 {
        // Empty tree - return zero hash at root level
        return Ok(tree.zero_hashes[depth - 1]);
    }
...
mod tests {
    ...
    #[test]
    fn test_zero_hashes() {
        let zero_hashes = compute_zero_hashes(MERKLE_DEPTH).unwrap();
        ...
        for i in 1..MERKLE_DEPTH {
            let expected = hash_two(&zero_hashes[i - 1], &zero_hashes[i - 1]).unwrap();
    ...
    fn test_insert_single_leaf() {
        let mut tree = PoolTree {
            ...
            roots_len: 0,
            roots_head: 0,
            zero_hashes: compute_zero_hashes_legacy(LEGACY_MERKLE_DEPTH).unwrap(),
            _padding_end: [0u8; 6],
        };
